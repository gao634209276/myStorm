...
/**
 * Storm实时计算集群从Kafka消息中间件中消费消息，有实时处理需求的可以走实时处理程序，还有需要进行离线分析的需求，
 * 如写入到HDFS进行分析。下面实现了一个Topology，代码如下所示：
 * <p>
 * 处理逻辑，可以对HdfsBolt进行更加详细的配置，
 * 如FileNameFormat、SyncPolicy、FileRotationPolicy
 * （可以设置在满足什么条件下，切出一个新的日志，如可以指定多长时间切出一个新的日志文件，
 * 可以指定一个日志文件大小达到设置值后，再写一个新日志文件），更多设置可以参考storm-hdfs。
 * <p>
 * 上面代码在打包的时候，需要注意，使用storm-starter自带的Maven打包配置，
 * 可能在将Topology部署运行的时候，会报错，可以使用maven-shade-plugin这个插件
 */
